...
class DBHelper(context: Context, factory: SQLiteDatabase.CursorFactory?) :
    ...
    var creationLogoQuery: String =
        "CREATE TABLE IF NOT EXISTS LOGO ( _ID INTEGER PRIMARY KEY AUTOINCREMENT, nombre TEXT, abreviatura TEXT, imagenURL TEXT)"
    var creationCarQuery: String =
        "CREATE TABLE IF NOT EXISTS CAR ( _ID INTEGER PRIMARY KEY AUTOINCREMENT, consumoCiudad REAL, descripcionAuto TEXT, consumoCombinado REAL, " +
        ...
        logos.forEach { logo ->
            if (chk.isNullOrEmpty() || chk.find { it.nombre == logo.nombre && it.abreviatura == logo.abreviatura && it.imagenURL == logo.imagenURL } == null) {
                ...
                val imageLink: String =
                    if (logo.imagenURL.isNullOrEmpty()) "" else logo.imagenURL.toString()
    ...
    fun addCars(cars: List<CarResponse>) {
        ...
        cars.forEach { car ->
            if (chk.isNullOrEmpty() || chk.find { it.modelo == car.modelo && it.marca == car.marca && it.combustible == car.combustible && it.ano == car.ano && it.motor == car.motor && it.cilindrada == car.cilindrada } == null) {
                val values = ContentValues()
                val cComb: Int =
                    if (car.consumoCombinado.toString().isNullOrEmpty()) 0 else car.consumoCombinado
                val mot: Int = if (car.motor.toString().isNullOrEmpty()) 0 else car.motor
                val cCiudad: Double =
                    if (car.consumoCiudad.toString().isNullOrEmpty()) 0.0 else car.consumoCiudad
                val cCilindrada: Double =
                    if (car.cilindrada.toString().isNullOrEmpty()) 0.0 else car.cilindrada
                val cAutopista: Double = if (car.consumoAutopista.toString()
                        .isNullOrEmpty()
                ) 0.0 else car.consumoAutopista
                val descAuto: String =
                    if (car.descripcionAuto.isNullOrEmpty()) "" else car.descripcionAuto
                val tConduccion: String =
                    if (car.tipoConduccion.isNullOrEmpty()) "" else car.tipoConduccion
                val tCombustible: String =
                    if (car.combustible.isNullOrEmpty()) "" else car.combustible
                ...
                val imageLink: String =
                    if (car.image.toString().isNullOrEmpty()) "" else car.image.toString()
